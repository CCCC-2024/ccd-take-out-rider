    ...
    <sanshuiModal ref="ssModal" :mode="_parentMode" :position="_parentPosition" @change="changeModal">
      <view class="digital-keyboard-modal" :class="'digital-keyboard-modal-' + _mode">
        <!-- <view class="dk-close " v-if="_mode !== 1 && _mode !== 4 && showClose" @click="close()">
          <text class="cuIcon-back fu-fs36"></text>
        </view> -->
        ...
        <block v-if="_mode !== 1 && _mode !== 4">
          ...
          <view class="pwd-tips">{{text}}</view> 
          <view class="pwd-tips pwd-tips-errot">{{ msgText }}</view>
        </view>
        </block>  
        <view class="pwd-forget"><text v-if="_forget && _mode === 1" @click="forgetPwd">忘记提现密码</text></view>
        <view class="digital-keyboard" :class="{ 'digital-keyboard-show': dkFlag || _mode === 1 || _mode === 4 }">
          <!-- <view class="dk-down" v-if="_mode !== 1 && _mode !== 4">
            ...
            <block v-if="trigger === 'auto'"><view class="dk-submit" @click="submit(true)">{{i18n['完成']}}</view></block>
          </view> -->
...
export default {
  ...
  methods: {
    ...
    close() {
      ...
      // let _this = this;
      // if (this._mode === 2) {
      //   tips = this.i18n['您是否要放弃设置支付密码'];
      // } else if (this._mode === 5) {
      //   tips = this.i18n['您是否要放弃重置支付密码'];
      // } else {
      //   tips = this.i18n['您是否要放弃修改支付密码'];
      // }
      // // #ifndef APP-PLUS
      // uni.showModal({
      //   title: _this.i18n['关闭提示'],
      //   content: tips,
      //   confirmText: _this.i18n['我再想想'],
      //   confirmColor: '#f02523',
      //   cancelText: _this.i18n['关闭'],
      //   success: res => {
      //     if (!res.confirm) {
      //       this.modalFun('hide');
      //       this.cancel();
      ...
      //         {
      //           title: _this.i18n['提示'],
      //           buttons: [_this.i18n['关闭'], _this.i18n['我再想想']]
      //         }
      //       );
      //     } else {
      //       plus.nativeUI.confirm(
      ...
      //         {
      //           title: _this.i18n['提示'],
      //           buttons: [_this.i18n['我再想想'], _this.i18n['关闭']]
      //         }
      //       );
      //     }
      //   }
    ...
    submit(clickFlag) {
      ...
      if (this.paymentPwd.length !== this._digits) {
        this.showError(this.i18n['请输入'] + this._digits + this.i18n['位支付密码']);
      } else {
        this.showError('');
