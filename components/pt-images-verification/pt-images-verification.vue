		...
		<view class="pt-verification-box">
			...
			<view class="pt-dragbar">
				...
				<movable-area class="pt-dragbar-area">
					<movable-view :class="['pt-dragbar-view',{active: dragWidth > 2,fail: fail,success: success}]"
						:direction="direction" @change="drag" @touchend="dragEnd" :damping="200" :x="x"
						:animation="false" :disabled="disabled" :data-dragWidth="dragWidth"><text class="iconfont">
		...
		methods: {
			...
			dragEnd(e) {
				...
				if (e.currentTarget.dataset.dragwidth > 0 && !this.disabled) {
					...
					if (!result) {
						this.fail = true;
						this.$emit('dragEnd', this.dragWidth)
						// uni.showToast({
						// 	icon: 'none',
						// 	title: '验证失败，再来一次吧',
						// 	duration: 1500
