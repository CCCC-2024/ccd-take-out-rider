<template>
  <view class="page">
    ...
    <!-- 数据加载动画 start -->
    <!-- <fu-loading v-if="isShowLoading"></fu-loading>   v-else-->
    <!-- 数据加载动画 end -->
    <block>
      <!-- 反馈列表 start -->
          ...
          <view class="flex item-list">
            ...
            <view class="item-desc">
              <view class="margin-bottom content-body flex">
                <view class="write-content" :class="[ (item.isMoreLine &&  item.isOpent) ? '' : '']"
                ...
                <!-- <view class="direction-position open_close flex flex-direction " v-if="item.isMoreLine" @click="hanldeOpenOrClose(item)"> 
                  ...
                  <image src="/static/img/pullDown.png" mode=""></image>
                </view> -->
              </view>
              ...
              <view class="concart"> {{LANGUAGE_DATA['联系方式'][LANGUAGE]}}:<text
                  class="margin-left-sm">{{item.contact}}</text></view>
              <view class="text-gray feedback-type">
              {{LANGUAGE_DATA['反馈类型'][LANGUAGE]}}:<text class="margin-left-sm">{{item.type}}</text></view>
              <view class="text-gray feedback-type">{{LANGUAGE_DATA['反馈时间'][LANGUAGE]}} :<text
                  class="margin-left-sm">{{item.create_time | formatDate('YYYY-MM-DD')}}
                </text></view>
      ...
      <view v-if="isInitEnd" class="text-xl text-center text-gray flex flex-direction align-center justify-center">
        ...
        <text class="margin-top text-df">{{LANGUAGE_DATA['暂无反馈内容'][LANGUAGE]}}</text>
      </view>
      <!-- 无数据 end -->
    </block>
    <!-- 断网监测 start -->
  ...
  export default {
    ...
    methods: {
      ...
      get_list() {
        ...
        this.$api.get(global.apiUrls.history).then(res => {
          console.log(res)
          if (res.data.code == 1) {
            this.list = res.data.data.data
            // JSON.parse(JSON.stringify(res.data.data));
            // 数据为空时显示空状态
            console.log('126------------',res.data.data.data)
