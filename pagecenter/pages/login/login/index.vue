	...
	<view class="page">
			...
			<view class="">
				<view class="item">
					...
					<view class="item_left">
						<view class="entry">
							<input type="number" :placeholder="LANGUAGE_DATA['请输入手机号'][LANGUAGE]" v-model="m_phone"
								style="width: 500rpx;" />
						</view>
					</view>
				</view>
					...
					<view class="item_left">
						<view class="entry">
							<input placeholder-class="input-placeholder" type="password"
								v-if="passwordInputType == 'password'" v-model="m_password"
							...
							<input placeholder-class="input-placeholder" v-if="passwordInputType == 'text'" type="text"
								v-model="m_password" :placeholder="LANGUAGE_DATA['请输入密码(6-12位数字+字母)'][LANGUAGE]"
				...
				<view class="switchLogin">
					<text v-if="isacount" @click="changeCut">{{LANGUAGE_DATA['验证码登录'][LANGUAGE]}}</text>
					<text v-else @click="changeCut">{{LANGUAGE_DATA['密码登录'][LANGUAGE]}}</text>
				</view>
			</view>
			<view class="login_sub" @click="login_all">{{LANGUAGE_DATA['立即登录'][LANGUAGE]}}</view>
			<view class="other_oprice">
				<view class="forget_pawword" v-if="isGrounding ==1" @tap="handleJump"
					data-url="/pagecenter/pages/login/register/index">{{LANGUAGE_DATA['注册账号'][LANGUAGE]}}</view>
				<view class="tips" v-if="isGrounding ==1"></view>
				<view class="forget_pawword" @tap="handleJump" data-url="/pagecenter/pages/login/forget_password/index">
		...
		<view class="" v-if="isGrounding ==1">
			<view class="quick">
				...
				<view class="sanfang">
					<image @click="wxLogin" src="/pagecenter/static/img/wxFirst.png" mode=""></image>
				</view>
			</view>
		</view>
		...
		<view class="agreen">
			<view class="agreen-style" @click="changeTick">
				<image src="/pagecenter/static/img/check_xy.png" mode="" class="chose-img" v-if="checked"></image>
	...
	export default {
		...
		methods: {
			...
			handleSendVerCode() {
				...
				if (!validate(m_phone, 'phone')) return this.$message.info('手机号格式不正确')
								...
								_this.$api.post(global.apiUrls.postUserSocialLogin, {
									...
									if (res.data.code == 1) {
										...
										if (res.data.data.userinfo.approve_status == -1) {
											// 已注册,未填写认证信息
											uni.navigateTo({
												url: '/pagecenter/pages/login/auth/index?token=' +
													res.data.data.userinfo
													.user_token
											})
										} else if (res.data.data.userinfo.approve_status ==
											0) {
											// 待审核
											...
											uni.navigateTo({
												url: '/pagecenter/pages/login/auth_wait/index?type=3' +
													'&token=' + that.loginData
													.user_token + '&reason=' + that
													.loginData.reason
											})
											...
											uni.setStorageSync('USER_INFO', global
												.userInfo)
											setTimeout(() => {
												uni.switchTab({
										...
										_this.$message.info(res.data.msg)
										uni.navigateTo({
											url: '/pagecenter/pages/login/bindPhone/index?un=' +
												infoRes.userInfo.unionId +
												'&open=' + infoRes.userInfo
												.openId + '&name=' + infoRes
												.userInfo.nickName + '&head=' +
			...
			login_all() {
				...
				if (m_phone == '') return this.$message.info(`${this.LANGUAGE_DATA['请输入手机号'][this.LANGUAGE]}`);
				if (!validate(m_phone, 'phone')) return this.$message.info('手机号格式不正确')
				if (!this.checked) return this.$message.info('请同意并勾选《用户协议》和《隐私协议》')
				if (isacount) {
					console.log('密码登录')
					this.psdLogin()
				} else {
			...
			psdLogin() {
				...
				if (m_password == '') return this.$message.info(
					`${this.LANGUAGE_DATA['请输入密码(6-12位数字+字母)'][this.LANGUAGE]}`);
				if (!validate(m_password, 'password')) return this.$message.info(
					`${this.LANGUAGE_DATA['密码格式错误(6-12位数字及字母)'][this.LANGUAGE]}`);
				console.log(clientId)

				this.$api.post(global.apiUrls.psdLogin, {
					mobile: m_phone,
					password: m_password,
					client_id: clientId,
					...
					if (res.data.code == 1) {
						...
						if (res.data.data.userinfo.approve_status == -1) {
							// 已注册,未填写认证信息
							uni.navigateTo({
								url: '/pagecenter/pages/login/auth/index?token=' + res.data.data.userinfo
									.user_token
							})
							...
							// this.$message.info(`${this.LANGUAGE_DATA['认证信息审核未通过，请重新填写信息'][this.LANGUAGE]}`)
							uni.navigateTo({
								url: '/pagecenter/pages/login/auth_wait/index?type=3' + '&token=' + that
									.loginData.user_token + '&reason=' + that.loginData.reason
							})
			...
			codeLogin() {
				...
				this.$api.post(global.apiUrls.codeLogin, {
					mobile: m_phone,
					client_id: clientId,
					app_system: appSystem,
					code: verifCode
